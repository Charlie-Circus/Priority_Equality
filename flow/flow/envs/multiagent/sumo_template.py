...
YAML_FILE_PATH = 'Dublin_phase_lane.yaml'
HDC_w = 0.2
RL_w = 0.8
...
class CoTVCustomEnv(MultiEnv):
    ...
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        ...
        #看看是啥
        print(self.mapping_inc)
        print(self.mapping_out)
        print(self.num_local_edges_max)
        print(self.num_out_edges_max)
    ...
    def get_state(self):
        ...
        for tl_id in self.k.traffic_light.get_ids():
            ...
            tl_info = self.k.trafficlight.getCompleteRedYellowGreenDefinition(tl_id)
            phases = tl_info[0].phases
            # 在YAML字典中查找对应的数据
            traffic_light_data = data['traffic_lights_ID_Dublin_selected'].get(tl_id, {})
            for phase_index, phase in enumerate(phases):
                phase_state = phase.state
                lanes = traffic_light_data.get(phase_state, [])
                phase_pri = 0
                #遍历lanes列表，统计其中的车辆
                for lane_id in lanes:
                    vehicle_ids = self.k.lane.getLastStepVehicleIDs(lane_id)
                    for veh_id in vehicle_ids:
                        veh_type = self.k.vehicle.getTypeID(veh_id) 
                        if veh_type == 'HDC':
                            phase_pri += HDC_w 
                        elif veh_type == 'rl':
                            phase_pri += RL_w
                tl_lanes_mov_pri.append(phase_pri)
